...
// ── Lecture publique ────────────────────────────────────────────────────────
router.get('/', (_req, res) => {
  res.json(db.prepare('SELECT * FROM resources ORDER BY id ASC').all())
})
...
router.post('/', requireAuth, (req, res) => {
  ...
  const result = db
    .prepare('INSERT INTO resources (title, category, type, date, size, url, content) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  db.prepare(
    'UPDATE resources SET title = ?, category = ?, type = ?, date = ?, size = ?, url = ?, content = ? WHERE id = ?'
  ).run(
