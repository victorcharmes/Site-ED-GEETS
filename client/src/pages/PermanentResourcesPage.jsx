...
const fallbackResources = [
  { id: 1, title: "Charte des permanents de l'Université de Toulouse", category: 'Réglementation', type: 'pdf', date: 'Mars 2023', size: '1.2 MB', url: '', content: "Charte encadrant les droits et devoirs des permanents et de leurs responsables." },
  { id: 2, title: 'Guide de recrutement des permanents', category: 'Général', type: 'texte', date: 'Janvier 2024', size: '15 KB', url: '', content: "Présentation des principales étapes de recrutement et d'accueil des personnels permanents : publication du poste, sélection, formalités administratives, prise de fonctions et contacts utiles." },
  { id: 3, title: "Référentiel d'accueil administratif des permanents", category: 'Suivi', type: 'pdf', date: 'Février 2024', size: '450 KB', url: '', content: "Document de suivi pour les démarches administratives des nouveaux permanents : contacts RH, accès aux outils, badges, messagerie et services internes." },
  { id: 4, title: "Procédure d'arrivée et d'installation", category: 'Inscription', type: 'texte', date: 'Septembre 2023', size: '12 KB', url: '', content: "Étapes d'arrivée pour les permanents :\n- Finaliser les formalités administratives auprès des services compétents.\n- Demander les accès informatiques et l'ouverture des droits.\n- Récupérer les moyens d'accès aux locaux.\n- Consulter les documents d'accueil et les contacts de référence." },
  { id: 5, title: 'Catalogue des formations pour les permanents', category: 'Formation', type: 'pdf', date: 'Novembre 2023', size: '2.1 MB', url: '', content: "Catalogue des formations professionnelles ouvertes aux permanents : prise de poste, outils numériques, prévention, management et développement des compétences." },
  { id: 6, title: 'Vade-Mecum du permanent GEETS', category: 'Général', type: 'texte', date: 'Octobre 2023', size: '18 KB', url: '', content: "Guide pratique : accès aux locaux, informatique, congés, missions, publications et affiliation." },
]
  ...
  const handleSubmit = async (e) => {
    ...
    if (type === 'pdf' && fileMode === 'upload' && selectedFile) {
      ...
      try {
        ...
        const res = await fetch('/api/permanent-resources/upload', {
          method: 'POST',
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
          ...
          <label className="flex flex-col gap-1.5 text-sm font-semibold text-slate-700">
            ...
            <input
              ...
              placeholder="Ex : Charte des permanents…"
            />
          </label>
...
export default function PermanentResourcesPage() {
  ...
  const permHeader = useEditableText('permanent_resources_page_header', "Consultez l'ensemble des fiches, documents et formulaires à destination du personnel permanent de l'école doctorale.")
  ...
  const handleSave = async (data) => {
    ...
    try {
      const isNew = !data.id
      const res = await fetch(isNew ? '/api/permanent-resources' : `/api/permanent-resources/${data.id}`, {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(id)
    try {
      await fetch(`/api/permanent-resources/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      })
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24">
          <nav className="flex text-sm text-univ-200 mb-6 font-medium" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-2">
              ...
              <li className="text-white" aria-current="page">Ressources Permanents</li>
            </ol>
          </nav>
            ...
            <div>
              ...
              {permHeader.editing ? (
                <div className="mt-1">
                  ...
                  <textarea value={permHeader.editValue} onChange={e => permHeader.setEditValue(e.target.value)} rows={2}
                  ...
                  <div className="flex gap-3 mt-2">
                    <button onClick={permHeader.save} disabled={permHeader.saving}
                      ...
                      <Check className="h-3.5 w-3.5" />{permHeader.saving ? 'Enregistrement...' : 'Enregistrer'}
                    </button>
                    <button onClick={permHeader.cancelEdit}
                ...
                <div className="flex items-start gap-3">
                  <p className="text-xl text-univ-100 leading-relaxed max-w-2xl">{permHeader.text}</p>
                  {isAdmin && (
                    <button onClick={permHeader.startEdit}
